...
worker_class = "sync"
# 900s, not AlphaFraud's 60s -- a sync worker here blocks on both a large (up to
# 200MB) upload body AND the subsequent subprocess.run() for `analyze` (whose own
# runner.py timeout is 900s, to cover PLIP interaction detection + compare-sse's
# external API lookups per target; gunicorn's worker timeout must be at least that
