...
**A construct numbered differently from its reference still compares.** The receptor
superposition matches residues by number and requires the residue types to agree, which is the
right correspondence when both sides number the same protein the same way -- and is not the case
nearly as often as it sounds. Boltz numbers a prediction from 1, while a PDB entry numbers the
construct it holds, so a trimmed ABL1 kinase domain came out 1-272 against references numbered
223-500, shared not one residue number with either, and every pose in the campaign went
uncompared behind the message "2GQG is not a structure of ABL1". A trim is a contiguous slice, so
one integer reconciles the two, and it is found rather than assumed: every offset that could
bring the ranges into contact is scored by how many residues then agree in type, and the best
wins. Matching numbering still gives offset zero, because that is what matches most. Nothing is
compared where the residues do not correspond, which is the property the bare number-matching
had and the reason an alignment was not used instead.
...
- [x] Superpose a prediction on a reference that **numbers its residues differently**. Matching on number assumed both sides number the same protein the same way; Boltz numbers a prediction from 1, so a trimmed kinase domain shared no residue with references numbered 223-500 and the pose panel reported "2GQG is not a structure of ABL1" while silently comparing nothing. The offset is now found by which shift makes the residue types agree, so a trimmed or engineered construct compares like any other and matching numbering still wins with offset zero. Measured on the trimmed ABL1 campaign: unconstrained imatinib reads 1.31A against 1IEP where the full-length construct gave 18.39A.
- [x] Stop reporting a **missing interaction analysis as a finding of no contacts**. cif2plip is optional, so a campaign run without it produces no contacts for any target, and the viewer said "0 contacting residues and the ligand as sticks" -- an absence of analysis stated as a fact about the ligand, which are opposite claims. The results file now records whether the analysis ran at all, and the viewer says which case it is.
